...
// GET /dashboard/api/bus-stops - Get all bus stops
export async function GET(request: NextRequest) {
  try {
    ...
    // Check if user is CONTROL_ADMIN (for control-center endpoint)
    if (user?.role === 'CONTROL_ADMIN') {
      // Use control-center endpoint for admins
      console.log('Fetching bus stops from control-center endpoint');
      const backendResponse = await fetch(`${BACKEND_API_BASE_URL}/api/control-center/bus-stops`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      // Use general buses/stops endpoint for CONTROL_STAFF
      console.log('Fetching bus stops from general endpoint');
    ...
    console.error('Error in GET /dashboard/api/bus-stops:', error);
    return NextResponse.json(
...
// POST /dashboard/api/bus-stops - Create new bus stop
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error in POST /dashboard/api/bus-stops:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
