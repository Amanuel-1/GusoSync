...
export async function PUT(
  ...
  try {
    ...
    if (token) {
      try {
        const backendResponse = await fetch(`${BACKEND_API_BASE_URL}/api/notifications/${id}/read`, {
          method: 'PUT',
          headers: {
        ...
        if (backendResponse.ok) {
          ...
          console.log('Backend mark-as-read notification endpoint not available');
        }
      } catch (error) {
        console.log('Error calling backend mark-as-read notification:', error);
      }
    }
    ...
    console.error('Error in mark-as-read notification API:', error);
    return NextResponse.json(
      { 
