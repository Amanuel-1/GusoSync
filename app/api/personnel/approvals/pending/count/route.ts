...
// GET /api/personnel/approvals/pending/count - Get pending requests count
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_API_BASE_URL}/api/approvals/requests/pending/count`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (response.ok) {
      ...
      return NextResponse.json(
        { error: data.detail || 'Failed to fetch pending count' },
